...
test-db-ide:
	docker exec -i $(PHP_CONTAINER) php bin/console doctrine:database:drop --force --if-exists --env=test
	docker exec -i $(PHP_CONTAINER) php bin/console doctrine:database:create --env=test
	docker exec -i $(PHP_CONTAINER) php bin/console doctrine:migrations:migrate --no-interaction --env=test
	docker exec -i $(PHP_CONTAINER) php bin/console doctrine:fixtures:load --no-interaction --env=test
